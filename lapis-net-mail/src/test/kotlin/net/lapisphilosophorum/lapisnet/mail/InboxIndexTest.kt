...
class InboxIndexTest :
    FunSpec({
        ...
        test(
            ...
            val index = InboxIndex(maxTracked = 100, maxPersisted = 1)
            val a = freshMessage(1)
            val b = freshMessage(2)

            // Mirrors what InboxGossip.onGossipMessage does on a storage.put() failure: call
            ...
            index.tryReservePersistence(a.envelope) shouldBe true
            index.releaseReservedPersistence(a.envelope)

            // THE CENTRAL ASSERTION: a wholly different, later envelope can still claim the cap's ONE
            ...
            index.tryReservePersistence(b.envelope) shouldBe true
            ...
            index.releaseReservedPersistence(a.envelope) // already released above - must not throw or corrupt state
            index.tryReservePersistence(freshMessage(3).envelope) shouldBe false // cap still enforced
        }
