...
payment sending), BOLT-12 offer publication, NAT traversal hardening, calling
(V0.8.7+; V0.8's group-calling/MLS target was cut from the arc entirely on
2026-08-04), and offline/mailbox message redelivery (V0.8.5+). V0.8.1, the
signed peer/presence directory records these later sub-waves resolve
...
deferred, not built even as stubs), no offline/mailbox redelivery path
(V0.8.5's job — this wave defines a forward-compatible dedup key so that wave
does not have to retrofit one).
