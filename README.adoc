...
end-to-end messaging — Double Ratchet 1:1 sessions, online DM delivery, and
calling (V0.8.3+; V0.8's group-calling/MLS target was cut from the arc
entirely on 2026-08-04). V0.8.1, the signed peer/presence directory records
these later sub-waves resolve recipients' addresses through, is implemented —
see `lapis-net-directory` below. V0.8.2 — the X25519 encryption sub-key, the
X3DH handshake (Signal's Extended Triple Diffie-Hellman, reimplemented from
the public specification), and prekey bundle publication/consumption — is
also implemented, in the new `lapis-net-ratchet` module, but not yet wired
into a live message-send path (that is V0.8.3/V0.8.4's job).
...
| `lapis-net-ratchet`
| X25519 encryption sub-key and the X3DH handshake (Signal's Extended Triple Diffie-Hellman, reimplemented from the public specification): `PrekeyBundle`/`PrekeyBundleCodec`, `PrekeyStore` (durable one-time-prekey consumption), and the `X3dh` handshake itself. Deliberately network-free (V0.8.2).
